...
def run_simulation(btn2,btn3,dinamics_vital,network,answer,model):
    ...
    if (a1 and a2 and a3 and a4):
        ...
        if network.get() == "Random":
            popen('./models/ran ' + str(ans) + net_par).read()
        elif network.get() == "Small-World":
            popen('./models/smw ' + str(ans) + net_par).read()
        elif network.get() == "Scale-Free":
            popen('./models/scf ' + str(ans) + net_par).read()
        elif network.get() == "Modular":
            popen('./models/mod ' + str(ans) + net_par).read()
        elif network.get() == "Hierarchical":
            popen('./models/hie ' + str(ans) + net_par).read()
        else:
            popen('./models/other ' + '"' + filename[0] + '"').read()

        output = popen('./IBM_WRITER').read()
        ...
        if dinamics_vital.get() == "without":
            output = popen('./IBM_MAIN ' + a + ' ' + str(ans) + ' N').read()
        else:
            output = popen('./IBM_MAIN ' + a + ' ' + str(ans) + ' Y '+ str(value_dv[0].get())).read()
...
def plot_results(answer,model):
    ...
    plt.figure('graph results (fig' + str(len(plt.get_fignums()) + 1) + ')')
...
def show_measures():
    ...
    output = popen('./measures network.dat').read()
